...
from NeuroTalkDataset2 import myDataset
    ...
    with torch.no_grad():
        ...
        for i, (input, target, target_cl, voice, data_info) in enumerate(pbar):
            ...
            for batch_idx in range(len(input)):
                ...
                # --- 保存 目标音频 (Target) ---
                # 修改：将标签文本加入到 Target 文件名中
                title_target = f"Target_{str_tar}"
                wavio.write(os.path.join(args.savevoice, f"{save_idx:03d}_{title_target}.wav"),
                           wav_target_np[batch_idx], args.sample_rate_STT, sampwidth=1)

                # --- 保存 原始音频 (Original) ---
                # 修改：将标签文本加入到 Original 文件名中
                title_origin = f"Original_{str_tar}"
                # 旧脚本特征：硬编码采样率为 22050，sampwidth=1
                wavio.write(os.path.join(args.savevoice, f"{save_idx:03d}_{title_origin}.wav"),
                           voice_np[batch_idx], 22050, sampwidth=1) # 保留旧脚本硬编码特征
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataLoc', type=str, default='./processed_dataset_6/'+TEST_FILE)
    parser.add_argument('--config', type=str, default='./config_myPrivate1_newdata1.json')
    parser.add_argument('--logDir', type=str, default='./6_newdata2/'+TEST_FILE)
    ...
    #     "DAD|", "IS|", "A|", "POLICEMAN|", "HE|", "WILL|", "ALWAYS|", "BECOME|", "MY|", "HERO|"
    # ])
    parser.add_argument('--word_label', type=list, default=[
        "MOM|", "SAID|", "SHE|", "WOULD|", "COOK|", "DINNER|", "TONIGHT|"
    ])

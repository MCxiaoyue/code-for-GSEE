...
# 定义文件名模式
# Target 文件模式: 例如 001_Target_MY.wav 或 001_Target.wav
target_pattern = re.compile(r'^(\d+)_Target(?:_[^.]*)?\.wav$')  # 匹配 001 或 001_MY 后缀
# Recon 文件模式: 例如 001_Recon_SP_MY.wav
recon_pattern = re.compile(r'^(\d+)_Recon_SP_.*\.wav$')
# ===========================================
...
all_wav_files = [f for f in all_files if f.lower().endswith('.wav')]

print(f"Found {len(all_wav_files)} total .wav files.")
# 根据模式分离 Target 和 Recon 文件，并提取 ID
target_files_by_id = {}
recon_files_by_id = {}

for f in all_wav_files:
    target_match = target_pattern.match(f)
    if target_match:
        id_num = target_match.group(1)  # 提取数字部分作为 ID
        target_files_by_id[id_num] = f
        continue

    recon_match = recon_pattern.match(f)
    if recon_match:
        id_num = recon_match.group(1)  # 提取数字部分作为 ID
        recon_files_by_id[id_num] = f
        continue

print(f"Found {len(target_files_by_id)} target files and {len(recon_files_by_id)} recon files.\n")

# 找出共同的 ID
common_ids = set(target_files_by_id.keys()).intersection(set(recon_files_by_id.keys()))
print(f"Found {len(common_ids)} matching ID pairs. Starting evaluation...\n")
...
for base_id in sorted(common_ids, key=int):  # 按数字大小排序，方便追踪
    target_file = target_files_by_id[base_id]
    recon_file = recon_files_by_id[base_id]

    target_path = os.path.join(data_folder, target_file)
    recon_path = os.path.join(data_folder, recon_file)
    ...
    try:
        ...
        print(
            f"Error processing ID '{base_id}' with files:\n  Target: {target_file}\n  Recon : {recon_file}\n  Error: {e}")
        import traceback

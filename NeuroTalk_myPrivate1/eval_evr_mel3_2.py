...
def generat(args, test_loader, models):
    ...
    with torch.no_grad():
        ...
        for i, (input, target, target_cl, voice, data_info) in enumerate(pbar):
            ...
            # --- 8. 保存三种音频文件 ---
            # 将 PyTorch tensor 转换为 numpy array 以便 wavio 保存
            wav_recon_np = np.squeeze(wav_recon.cpu().detach().numpy())
            wav_target_np = np.squeeze(wav_target.cpu().detach().numpy())
            # 原始语音数据也需要重采样到STT的采样率以便保存
            voice_for_save = torchaudio.functional.resample(voice_raw, args.sample_rate_mel, args.sample_rate_STT)
            voice_np = np.squeeze(voice_for_save.cpu().detach().numpy())

            # 保存生成的音频 (Reconstructed)
            wavio.write(os.path.join(args.savevoice, f'test_batch_{i}_Recon_{title}.wav'), wav_recon_np,
                        args.sample_rate_STT, sampwidth=1)
            # 保存目标音频 (Target)
            wavio.write(os.path.join(args.savevoice, f'test_batch_{i}_Target_{title}.wav'), wav_target_np,
                        args.sample_rate_STT, sampwidth=1)
            # 保存原始音频 (Original)
            wavio.write(os.path.join(args.savevoice, f'test_batch_{i}_Original_{title}.wav'), voice_np,
                        args.sample_rate_STT, sampwidth=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logDir', type=str, default='./TrainResult_EVR4')

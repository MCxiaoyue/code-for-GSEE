...
class Discriminator(torch.nn.Module):
    def __init__(self, h):
        ...
        # 【修复】conv_post 的输入通道数是 GRU 输出 (ch) 和残差块输出 (ch) 拼接后的总数 (ch * 2)
        # 原来的写法: Conv1d(ch, ch, ...)
        # 正确的写法: Conv1d(ch * 2, ch * 2, ...)
        # 或者，也可以将输出通道数设为 ch，这样最后卷积头的输入就是 ch
        # 这里选择后者，使结构更清晰
        self.conv_post = weight_norm(Conv1d(ch * 2, ch, 9, 1, padding=get_padding(9, 1)))

        # 【新增】Dropout 作为正则化
        self.dropout = nn.Dropout(0.1)

        # 新增：PatchGAN 风格的卷积头
        self.conv_adv = weight_norm(Conv1d(ch, 1, 3, 1, padding=get_padding(3, 1)))  # 输出真假分数的特征图
        self.conv_cls = weight_norm(Conv1d(ch, h.n_classes, 3, 1, padding=get_padding(3, 1)))  # 输出分类的特征图

        self.conv_pre.apply(init_weights)
        self.downs.apply(init_weights)
        self.conv_post.apply(init_weights)
        self.conv_adv.apply(init_weights)
        self.conv_cls.apply(init_weights)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_downsamples):
            ...
            x = xs / self.num_kernels
        x = F.leaky_relu(x)
        x_temp = x  # x_temp.shape: (B, ch, T_local)
        x = x.transpose(1, 2)  # x.shape: (B, T_local, ch)
        self.GRU.flatten_parameters()
        x, _ = self.GRU(x)  # x.shape: (B, T_local, ch//2 * 2) = (B, T_local, ch)
        x = x.transpose(1, 2)  # x.shape: (B, ch, T_local)
        # x_temp 和 x 都是 (B, ch, T_local)，拼接后变成 (B, ch * 2, T_local)
        x = torch.cat([x, x_temp], dim=1)

        # 【新增】在 GRU 后应用 Dropout
        x = self.dropout(x)

        # x.shape: (B, ch * 2, T_local)
        x = self.conv_post(x)  # x.shape: (B, ch, T_local)
        x = F.leaky_relu(x)

        # 【修改】使用卷积头而非全连接层
        validity_map = self.conv_adv(x)  # (B, 1, T_local)
        label_map = self.conv_cls(x)  # (B, n_classes, T_local)

        # 为了与旧版训练循环兼容，我们对输出进行全局平均池化
        # 这样返回的仍然是 (B, 1) 和 (B, n_classes)
        # 但内部判别逻辑已经从全局变成了局部感知
        validity = torch.sigmoid(torch.mean(validity_map, dim=2))  # (B, 1)
        label = F.softmax(torch.mean(label_map, dim=2), dim=1)  # (B, n_classes)
    ...
    def remove_weight_norm(self):
        ...
        remove_weight_norm(self.conv_pre)
        remove_weight_norm(self.conv_post)
        remove_weight_norm(self.conv_adv)
        remove_weight_norm(self.conv_cls)
        # remove_weight_norm(self.dropout)  # nn.Dropout 通常不使用 weight_norm，所以不需要移除

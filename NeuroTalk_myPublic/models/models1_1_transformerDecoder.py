...
class EVRNet_Generator(torch.nn.Module):
    def __init__(self, h):
        ...
        # --- 2. 【核心优化】用于时序重建的 Transformer Decoder ---
        self.d_model = 1024  # 设定一个合适的维度，与后续生成器输入匹配
        self.target_seq_len = 16  # 设定一个初始的时序长度，可以根据需要调整

        # Query: 从一个可学习的位置嵌入中生成，代表我们想要的时序位置
        self.pos_embed = nn.Parameter(torch.randn(1, self.target_seq_len, self.d_model))

        # Transformer Decoder Layer
        self.decoder_layer = nn.TransformerDecoderLayer(
            d_model=self.d_model,
            nhead=8,  # 多头注意力
            dim_feedforward=self.d_model * 2,  # 前馈网络隐藏层大小
            dropout=0.1,
            activation='relu',
            batch_first=True
        )
        ...
        # --- 3. 特征映射层 ---
        # 将 Mamba 的输出映射到 d_model 维度
        self.mamba_to_d_model = nn.Linear(32, self.d_model)  # Mamba 输出是 16 维

        # 将 Transformer 输出映射到生成器的输入维度
        self.dec_to_gen = weight_norm(
            Conv1d(self.d_model, 1024, kernel_size=1)  # 假设生成器第一层需要 1024 通道
        )
    ...
    def forward(self, x):
        ...
        # --- 2. 【核心优化】使用 Transformer Decoder 重建时序 ---
        # x: (B, 1, 16) -> (B, 16)
        x = x.squeeze(1)  # (B, 16)
        x = self.mamba_to_d_model(x)  # (B, d_model)

        # 扩展维度以匹配 target_seq_len
        x = x.unsqueeze(1).expand(-1, self.target_seq_len, -1)  # (B, target_seq_len, d_model)

        # 生成 Query (来自位置嵌入)
        query = self.pos_embed.expand(x.size(0), -1, -1)  # (B, target_seq_len, d_model)

        # 通过 Transformer Decoder
        # Key=Value=x (来自 Mamba 的信息)，Query=位置嵌入
        x = self.decoder(query, x)  # (B, target_seq_len, d_model)

        # --- 3. 转换格式并传给原生生成器 ---
        x = x.permute(0, 2, 1)  # (B, d_model, target_seq_len)
        x = self.dec_to_gen(x)  # (B, 1024, target_seq_len)

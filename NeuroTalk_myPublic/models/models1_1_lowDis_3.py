...
class ResBlock(torch.nn.Module):
    def __init__(self, h, channels, kernel_size=3, dilation=(1, 3, 5)):
        ...
        self.h = h
        # 1. 【修改】: 用 spectral_norm 替换 weight_norm
...
class Discriminator(torch.nn.Module):
    def __init__(self, h):
        ...
        for i, (u, k) in enumerate(zip(h.downsample_rates, h.downsample_kernel_sizes)):
            self.downs.append(spectral_norm(
        ...
        for i in range(self.num_downsamples):
            ...
            for j in range(self.num_kernels):
                dilation_index = i * self.num_kernels + j
                if dilation_index < len(h.resblock_dilation_sizes):
                    current_dilation_config = h.resblock_dilation_sizes[dilation_index]
                else:
                    current_dilation_config = h.resblock_dilation_sizes[-1]
        ...
        final_channels_before_pool = gru_input_ch * 2
        ...
        # ================= 修改点 1：定义 Dropout 层 =================
        # p=0.2 或 0.3 比较合适，既能防止过拟合，又不会丢失太多信息
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            # 方法 1: 高斯噪声 (最常用)
            # noise_level 控制噪声强度，建议从 0.01 开始尝试
            # 0.01 意味着在 [-0.02, 0.02] 范围内抖动，对于归一化到 [-1, 1] 的音频很安全
            noise_level = 0.01
            x = x + torch.randn_like(x) * noise_level

            # 方法 2 (备选): 二值噪声/恒定方差
            # x = x + torch.sign(x) * 0.01 # 如果高斯噪声效果不好，可以试试这个
        ...
        # GRU 处理
        x_gru_in = x
        x_gru_in = x_gru_in.transpose(1, 2)
        self.GRU.flatten_parameters()
        x_gru_out, _ = self.GRU(x_gru_in)
        x_gru_out = x_gru_out.transpose(1, 2)
        # ================= 新增：对 GRU 输出应用 Dropout =================
        x_gru_out = self.gru_dropout(x_gru_out)

        # Concatenate
        x = torch.cat([x_gru_out, x], dim=1)

        # Pooling
        x = F.adaptive_avg_pool1d(x, self.pooled_length)

        # Flatten
        x = x.view(-1, self.fc_input_features)

        # ================= 修改点 2：在 FC 层前应用 Dropout =================
        x = self.dropout(x)
...
#         self.h = h
#         self.ch_init_downsample = h.ch_init_downsample
#         self.num_kernels = len(h.resblock_kernel_sizes)  # Kernels per downsample layer
#         self.num_downsamples = len(h.downsample_rates)  # Number of downsample layers
#         self.n_classes = h.n_classes
#         self.input_size = h.input_size
#
#         # 动态计算总的下采样倍率
...
#                        k, u, padding=math.ceil((k - u) / 2))))
#
#         # 创建 ResBlocks (完全动态适应配置)
#         # 关键修改点：为每个下采样层和每个kernel分配一个dilation配置
#         self.resblocks = nn.ModuleList()
#
#         # 遍历每一层下采样
#         for i in range(self.num_downsamples):
#             ch = h.ch_init_downsample * (2 ** (i + 1))  # 当前层的通道数
#
#             # 遍历当前下采样层的每一个kernel
#             for j in range(self.num_kernels):
#                 # 从 h.resblock_dilation_sizes 中按顺序取出一个dilation配置
#                 # 确保 len(h.resblock_dilation_sizes) >= num_downsamples * num_kernels
#                 # 例如，如果有 2 层，每层 3 个 kernels，则需要 6 个 dilation 配置
#                 dilation_index = i * self.num_kernels + j
#
#                 if dilation_index < len(h.resblock_dilation_sizes):
#                     current_dilation_config = h.resblock_dilation_sizes[dilation_index]
#                 else:
#                     # 如果配置不够，使用最后一个作为兜底，或抛出错误
#                     # 这里我们选择使用最后一个，以提供最大容错性
#                     current_dilation_config = h.resblock_dilation_sizes[-1]
#
